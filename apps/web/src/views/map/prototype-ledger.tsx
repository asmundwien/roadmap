...
import { buildLedger, type Ledger, type LedgerEdge } from './prototype-geometry.ts'
import './map.css'
import './prototype-map.css'
import { STATE_META } from './state-meta.ts'

/**
 * PROTOTYPE — throwaway fork of ledger.tsx for the single-map representation rework. The deltas
 * from production:
 *
 * - Titles only. Ground-covered rows drop their gist line and sit on the tighter single-line
 *   pitch (prototype-geometry.ts); the prose lives in the drawer now.
 * - Rows are not GitHub links any more — clicking one reports a selection upward and the drawer
 *   opens it; "View item in GitHub" lives inside the drawer.
 * - Out-of-scope joins the fog band as ⊘ stops — but only inline while the list is small. A vast
 *   list collapses to ONE aggregate ⊘ stop carrying the count, so scope can never drown the fog;
 *   the drawer holds the full list.
 * - Fog and scope stops are clickable too: their full text opens in the drawer.
 *
 * Everything else is a verbatim copy; diff against ledger.tsx to see the delta.
 */

/** What a click on the map means — the child turns this into drawer content. */
 ...
 * Needs `overflow: visible` on the svg (prototype-map.css); the map column's own overflow does
 * the clipping at exactly the panel edge. */
const EXT = 800
...
function scopePlan(map: WayfinderMap): ScopePlan {
...
export function PrototypeMapLedger({
  map,
  trunkToEdge = false,
  onSelect,
  selected,
  ...
  return (
    <div className="ledger-wrap">
      ...
      >
        ...
        {/* fog rows — ghost stops, now clickable: the drawer holds each one's full text. A small
        ...
        {ledger.fogRows.map(({ item, x, y }) => {
          ...
          const tip =
            sel.kind === 'scope-all'
              ? 'left out of scope — the full list opens in the drawer'
              : sel.kind === 'scope'
          ...
          return (
            <g
              key={item}
              className={`pfl-ghost-row${isSelected ? ' is-selected' : ''}`}
            ...
            >
              ...
              <text
                x={ledger.textX}
                y={y + 4}
                className={`fog-title${sel.kind === 'scope' ? ' pfl-scope-title' : ''}`}
              >
                {truncate(item, 52)}
              </text>
            </g>
        ...
        {/* ahead rows — the title and the state word; everything else waits in the drawer.
        ...
        {/* ground covered — title only, one line; the decision's gist moved to the drawer */}
 ...
 * rather than a shrug. PROTOTYPE delta: positioned off the title's baseline, so it rides one-line
 * and two-line rows alike.
 */
function TypeChip({
  type,
  title,
