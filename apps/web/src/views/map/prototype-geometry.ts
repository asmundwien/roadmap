...
/**
 * PROTOTYPE — throwaway fork of geometry.ts for the single-map representation rework. The delta:
 * per-section row pitch. Fog rows and (now single-line) ground-covered rows tighten to 40 units
 * while open rows keep 52 for their two text lines — the accidental airiness of one-line rows on
 * a two-line pitch was the complaint. Everything else is a verbatim copy; diff against
 * geometry.ts to see the delta.
 *
...
const ROW_H = 52
/** PROTOTYPE: single-line rows — fog ghosts and title-only covered ground — on their own pitch. */
const THIN_ROW_H = 40
const SEC_PAD = 56
const SEC_BOTTOM = 34
const PAD_TOP = 20
